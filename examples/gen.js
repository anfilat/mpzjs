...
} while (p.lt(MPZ.pow(2, L - 1)) || p.probPrime(50) === false);

assert.ok(q.gt(MPZ.pow(2, 159)), 'q > 2**159');
assert.ok(q.lt(MPZ.pow(2, 160)), 'q < 2**160');
assert.ok(p.gt(MPZ.pow(2, L - 1)), 'p > 2**(L-1)');
assert.ok(q.lt(MPZ.pow(2, L)), 'p < 2**L');
